...
def construct_curve_from_vector(vec, start_point, is_numerical=True):
    ...
    if type == LINE_IDX:
        ...
        if res is None: # 为了可视化目的，用直线替换错误的圆弧
            return Line.from_vector(vec, start_point, is_numerical=is_numerical)
        return res
    else:
...
class Line(CurveBase):
    ...
    def __str__(self):
        return "Line: start({}), end({})".format(self.start_point.round(4), self.end_point.round(4))#round(4)即保留4位小数

    @staticmethod
    def from_dict(stat):
        assert stat['type'] == "Line3D"#assert用于判断某个表达式，在表达式条件为 false 的时候触发异常
    ...
    def numericalize(self, n=256):
        self.start_point = self.start_point.round().clip(min=0, max=n-1).astype(np.int_)
        self.end_point = self.end_point.round().clip(min=0, max=n-1).astype(np.int_)
...
class Arc(CurveBase):
    ...
    def get_mid_point(self):
        assert self.start_angle is not None and self.end_angle is not None and self.ref_vec is not None
    ...
    def numericalize(self, n=256):
        self.start_point = self.start_point.round().clip(min=0, max=n-1).astype(np.int_)
        self.mid_point = self.mid_point.round().clip(min=0, max=n-1).astype(np.int_)
        self.end_point = self.end_point.round().clip(min=0, max=n-1).astype(np.int_)
        self.center = self.center.round().clip(min=0, max=n-1).astype(np.int_)
        tmp = np.array([self.start_angle, self.end_angle])
        self.start_angle, self.end_angle = (tmp / (2 * np.pi) * n).round().clip(
                                            min=0, max=n-1).astype(np.int_)
...
class Circle(CurveBase):
    ...
    def __str__(self):
        return "Circle: center({}), radius({})".format(self.center.round(4), round(float(self.radius), 4))
    ...
    def numericalize(self, n=256):
        self.center = self.center.round().clip(min=0, max=n-1).astype(np.int_)
        self.radius = np.round(self.radius).clip(min=1, max=n-1).astype(np.int_)
    ...
    def draw(self, ax, color):
        ap = patches.Circle((self.center[0], self.center[1]), float(self.radius),
