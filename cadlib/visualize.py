...
def create_CAD(cad_seq: CADSequence):
    """从 CADSequence 创建 3D CAD 模型。仅支持带布尔运算的挤出。"""
...
def create_by_extrude(extrude_op: Extrude):
    """从 Extrude 实例创建实体。"""
    profile = copy(extrude_op.profile) # 使用 copy 防止在内部更改 extrude_op
...
def create_profile_face(profile: Profile, sketch_plane: CoordSystem):
    ...
    for loop in all_loops[1:]:
        # Ensure loop is a TopoDS_Wire before passing to Add
        reversed_wire = loop.Reversed() if hasattr(loop, 'Reversed') else loop
        if hasattr(topo_face, 'Add'):
            topo_face.Add(reversed_wire)
        else:
            # fallback: try to get wire if loop is a shape
            try:
                topo_face.Add(reversed_wire.Wire())
            except Exception:
                raise TypeError("loop is not a TopoDS_Wire and cannot be added as a wire")
